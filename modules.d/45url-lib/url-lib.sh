...
basicauth=$(getarg basicauth=)
[ -n "$basicauth" ] && curl_args="$curl_args -u $basicauth"
...
file_fetch_url() {
    ...
    file="/${url##*//}"
    dev="${url%%//*}"
    mkdir -p /run/url_lib_file || return 253
    mount -t auto -o "$dev" /run/url_lib_file || return 253
    cp -f -- "/run/url_lib_file/$file" "$outloc" || return $?
    if [ -z "$2" ]; then echo "$outloc"; fi
}

add_url_handler file_fetch_url file
