...
if getargbool 0 debug -n -y rdlivedebug; then
...
if [ -n "$FSIMG" ]; then
    if [ -n "$writable_fsimg" ]; then
        ...
        echo "Unpacking live filesystem (may take some time) - vauban edition" > /dev/kmsg
        ...
        echo "live filesystem unpacked !" > /dev/kmsg
...
if [ -n "$overlayfs" ]; then
    ...
    echo "vauban: systemd" > /dev/kmsg
    if [ -z "$DRACUT_SYSTEMD" ]; then
        [ -n "$ROOTFLAGS" ] && ROOTFLAGS="-o $ROOTFLAGS"
        printf 'mount %s /dev/mapper/live-rw %s\n' "$ROOTFLAGS" "$NEWROOT" > $hookdir/mount/01-$$-live.sh
    fi
    echo "vauban: systemd done" > /dev/kmsg
fi
...
need_shutdown
echo vauban: mount points size in MiB > /dev/kmsg
...
echo vauban: root mapped > /dev/kmsg
exit 0
